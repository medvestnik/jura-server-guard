...
class Application
{
    ...
    private function validateScanSitePath(?string $path): void
    {
        ...
        $roots = array_map(fn($r) => rtrim(realpath($r) ?: $r, '/'), (array)config('guard.scan_roots'));
        foreach ($roots as $root) if ($root !== '' && ($real === $root || str_starts_with($real, $root.'/'))) return;
        throw new \InvalidArgumentException("scan-site path is outside configured scan roots: {$real}");
    }
    ...
    private function scanOptions(array $argv): array {
        $this->validateOptions($argv, ['--profile','--force','--no-lock','--include-old','--include-storage','--include-backups','--dry-run','--verbose','--max-files','--max-seconds','--diff','--changed-only','--full-rescan','--skip-logs','--include-logs','--lock-label','--paranoid']);
