...
class ScannerService
{
    ...
    // Must include is_missing=1 rows: a path_hash already has a UNIQUE key in file_snapshots, so a
    // file that was previously deleted (is_missing=1) and later reappears has to be found here and
    // routed through snapshot()'s UPDATE branch (which clears is_missing) -- excluding those rows
    // left the manifest diff treating the reappeared file as brand new, and the resulting INSERT
    // collided with the still-existing is_missing=1 row's path_hash (SQLSTATE[23000] 1062). The
    // $deleted computation right after the caller's diff loop already assumes missing rows are
    // present here (it checks empty($row['is_missing']) itself), so this was the only place the
    // filter needed to be removed.
    private function loadSnapshotMap(int $siteId): array { $rows=DB::select('SELECT * FROM file_snapshots WHERE site_id=?', [$siteId]); $m=[]; foreach($rows as $r) $m[$r['path_hash']]=$r; return $m; }
