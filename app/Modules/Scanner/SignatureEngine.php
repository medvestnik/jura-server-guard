...
class SignatureEngine
{
    ...
    /** Recognized combo pattern keys. A combo signature whose pattern_json uses none of these
     *  (e.g. a malformed/typo'd shape like {"match":"all","substrings":[...]}) is a data bug, not
     *  an intentional "match everything" rule -- fail closed instead of silently matching every
     *  targeted file. See incident: signatures #221-224 all shipped with an unrecognized
     *  {"match":...,"substrings":[...]} shape and matched every file of their target extension
     *  server-wide (tens of thousands of false "critical" findings) until this guard was added. */
    private const COMBO_RECOGNIZED_KEYS = ['all','regex_all','any','regex_any','not','required_groups'];
    private function matchCombo(array $p, string $content, string $pathText, array &$matched): bool { if (!array_intersect(array_keys($p), self::COMBO_RECOGNIZED_KEYS)) return false; foreach((array)($p['all']??[]) as $n) if(stripos($content,(string)$n)===false) return false; else $matched[]=['name'=>'all','pattern'=>(string)$n,'snippet'=>(string)$n]; foreach((array)($p['regex_all']??[]) as $r) if(!@preg_match($r,$content,$m)) return false; else $matched[]=['name'=>'regex_all','pattern'=>$r,'snippet'=>substr($m[0]??'',0,300)]; if(isset($p['any']) || isset($p['regex_any'])) { $before=count($matched); $okAny = isset($p['any']) && $this->any((array)$p['any'],$content,$matched); $okRegexAny = isset($p['regex_any']) && $this->regexAny((array)$p['regex_any'],$content,$matched); if(!$okAny && !$okRegexAny) return false; } foreach((array)($p['not']??[]) as $n) if(stripos($content,(string)$n)!==false) return false; foreach((array)($p['required_groups']??[]) as $g){ $hits=0; foreach((array)($g['any']??[]) as $n) if(stripos($content,(string)$n)!==false){$hits++;$matched[]=['name'=>'required_group','pattern'=>(string)$n,'snippet'=>(string)$n];} foreach((array)($g['regex_any']??[]) as $r) if(@preg_match($r,$content,$m)){ $hits++; $matched[]=['name'=>'required_group_regex','pattern'=>$r,'snippet'=>substr($m[0]??'',0,300)]; } if($hits < (int)($g['min']??1)) return false; } return true; }
    ...
    private function matchStructural(array $p,string $path,string $rel,string $content,array &$matched): bool { $pathMatched=false; if (!empty($p['path_regex']) && @preg_match((string)$p['path_regex'], $rel, $m)) { $pathMatched=true; $matched[]=['name'=>'path_regex','pattern'=>(string)$p['path_regex'],'snippet'=>$m[0] ?? $rel]; } foreach((array)($p['directory_names']??[]) as $d) if(preg_match('#(^|/)'.preg_quote($d,'#').'(/|$)#i',$rel)){ $pathMatched=true; $matched[]=['name'=>'directory','pattern'=>$d,'snippet'=>$rel]; } foreach((array)($p['path_contains']??[]) as $d) if(stripos($rel,(string)$d)!==false){ $pathMatched=true; $matched[]=['name'=>'path','pattern'=>(string)$d,'snippet'=>$rel]; } if (isset($p['same_directory_min_extension_variants']) && isset($p['filename_stem_pattern'])) { $variants = $this->multiExtensionSprayVariants($p, $path); if ($variants !== null) { $pathMatched=true; $matched[]=['name'=>'multi_extension_spray','pattern'=>'sibling files sharing filename stem across >= '.(int)$p['same_directory_min_extension_variants'].' of the configured extensions','snippet'=>basename($path).' ('.implode(', ', $variants).')']; } } $contentMatched=$this->regexAny((array)($p['regex_any']??[]),$content,$matched); if (!empty($p['require_path_and_content'])) return $pathMatched && $contentMatched; return $pathMatched || $contentMatched; }

    /** "Multi-extension numeric-ID payload spray" detector: an exploitation dropper that writes the
     *  same payload under one numeric-looking filename stem across many case/extension variants in
     *  the same directory (123456.php, 123456.PHP, 123456.phtml, ...) to survive extension-based
     *  execution blocks. Returns the list of sibling extensions actually found on disk (>= the
     *  configured threshold) or null if this file doesn't look like part of such a spray. */
    private function multiExtensionSprayVariants(array $p, string $path): ?array
    {
        $stemPattern = (string)($p['filename_stem_pattern'] ?? '');
        $minVariants = (int)($p['same_directory_min_extension_variants'] ?? 0);
        $extensions = array_values(array_unique((array)($p['extensions'] ?? [])));
        if ($stemPattern === '' || $minVariants <= 0 || !$extensions) return null;
        $base = basename($path);
        $ext = pathinfo($base, PATHINFO_EXTENSION);
        $stem = pathinfo($base, PATHINFO_FILENAME);
        if ($ext === '' || $stem === '' || !@preg_match($stemPattern, $stem)) return null;
        if (!in_array($ext, $extensions, true)) return null;
        $dir = dirname($path);
        $found = [];
        foreach ($extensions as $candidateExt) if (@is_file($dir.'/'.$stem.'.'.$candidateExt)) $found[] = $candidateExt;
        return count($found) >= $minVariants ? $found : null;
    }
}
