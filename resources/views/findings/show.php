...
<div class="card">
  ...
  <div class="actions">
    ...
    <?php if(config('guard.web_actions_enabled') && $finding['status']!=='quarantined'): ?><form method="post" action="/finding/quarantine" class="inline-form" onsubmit="return confirm(<?= e(json_encode(t('Move file to quarantine?'))) ?>)"><input type="hidden" name="id" value="<?= e($finding['id']) ?>"><button class="btn danger"><?= e(t('Quarantine')) ?></button></form><?php endif ?>
  </div>
