...
<?php if (in_array(($_GET['block_result'] ?? ''), ['blocked','network_blocked'], true)): ?><div class="notice success"><?= e(t('Network firewall rule installed. Direct traffic is blocked; a proxy/CDN may still forward this visitor IP.')) ?></div><?php endif ?>
<?php if (in_array(($_GET['block_result'] ?? ''), ['blocked','network_blocked','already_blocked'], true) && ($_GET['block_message'] ?? '') !== ''): ?><div class="notice danger-notice"><?= e(t('The active rule works, but persistence failed:')) ?> <?= e($_GET['block_message']) ?></div><?php endif ?>
<?php if (($_GET['block_result'] ?? '') === 'already_blocked'): ?><div class="notice info"><?= e(t('The direct-traffic firewall rule was already active.')) ?></div><?php endif ?>
...
<div class="table-scroll"><table class="suspicious-logs-table">
  ...
  <?php $threatIps=$threatIps??[]; foreach($events as $l): $ti=$threatIps[$l['ip']]??null; $rowId='log-'.$l['id']; $detailsId=$rowId.'-details'; $siteLabel=log_event_site_label($l); $networkBlocked=$ti&&in_array(($ti['firewall_status']??''),['blocked','network_blocked'],true); $afterBlock=$networkBlocked&&!empty($ti['blocked_at'])&&strtotime((string)($l['created_at']??''))>strtotime((string)$ti['blocked_at']); ?>
  <tr id="<?= e($rowId) ?>">
    ...
    <td><b class="nowrap"><?= e($l['ip']) ?></b><?php if($ti): ?><br><span class="badge <?= e($ti['risk']) ?>" title="<?= e(t($ti['classification'])) ?>"><?= e(t($ti['classification'])) ?></span><?php endif ?><?php if($networkBlocked): ?><br><span class="badge low"><?= e(t('Network rule active')) ?></span><?php if($afterBlock): ?><br><span class="badge critical" title="<?= e(t('The request was logged after the network firewall rule was installed. It probably arrived through a reverse proxy or CDN.')) ?>"><?= e(t('Request after block')) ?></span><?php endif ?><?php elseif(config('guard.firewall_actions_enabled')): ?><form method="post" action="/threat-ips/block" class="log-action-form" onsubmit="return submitIpBlock(this,<?= e(json_encode(t('Block this IP in the server firewall?'))) ?>)"><input type="hidden" name="ip" value="<?= e($l['ip']) ?>"><input type="hidden" name="log_event_id" value="<?= e($l['id']) ?>"><input type="hidden" name="classification" value="<?= e($ti['classification']??'scanner') ?>"><input type="hidden" name="risk" value="<?= e($ti['risk']??$l['risk']??'high') ?>"><input type="hidden" name="return_to" value="<?= e($_SERVER['REQUEST_URI']??'/logs') ?>"><button class="btn danger small"><?= e(t('Block IP')) ?></button></form><?php endif ?></td>
