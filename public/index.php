...
if ($path === '/scan/start' && $method==='POST') { $profile=in_array($_POST['profile']??'fast',['fast','standard','deep'],true)?$_POST['profile']:'fast'; $scope=in_array($_POST['scope']??'full',['full','user','site'],true)?$_POST['scope']:'full'; $value=$_POST['value']??null; $active=scan_active_context(); if($active['running']) redirect('/?scan_blocked=1'); start_background_scan($scope,$value,$profile); redirect('/'); }
if ($path === '/scan/stop' && $method==='POST') { $active=scan_active_context(); $run=$active['run']; $pid=(int)($run['pid'] ?? $active['lock']['pid'] ?? 0); if($pid && scan_pid_alive($pid) && function_exists('posix_kill')) @posix_kill($pid, SIGTERM); if($run) DB::statement('UPDATE scan_runs SET status=?, finished_at=?, error_text=?, progress_message=?, updated_at=? WHERE id=?', ['stopped', now(), 'stopped by admin', 'stopped by admin', now(), $run['id']]); (new ScanLock())->unlock(true); redirect('/scan/active'); }
if ($path === '/scan/force-unlock' && $method==='POST') { $run=DB::first("SELECT * FROM scan_runs WHERE status='running' ORDER BY id DESC LIMIT 1"); if(scan_is_stale($run)) { DB::statement('UPDATE scan_runs SET status=?, finished_at=?, error_text=?, progress_message=?, updated_at=? WHERE id=?', ['failed', now(), 'force unlocked stale scan by admin', 'force unlocked stale scan by admin', now(), $run['id']]); (new ScanLock())->unlock(true); } redirect('/scan/active'); }
...
$data = match ($path) {
 '/scan/active' => ['dashboard.index', (function(){ $ctx=scan_active_context(); return ['activeOnly'=>true,'activeScan'=>$ctx['run'],'activeLock'=>$ctx['lock'],'last'=>DB::first('SELECT * FROM scan_runs ORDER BY id DESC LIMIT 1'),'users'=>0,'sites'=>0,'new'=>0,'crit'=>0,'high'=>0,'q'=>0,'logs'=>[],'scanRuns'=>DB::select('SELECT * FROM scan_runs ORDER BY id DESC LIMIT 10')]; })()],
